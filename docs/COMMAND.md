...
# Движение нескольких моторов (multi-move)
poetry run python scripts/cli.py multi-move "1:500:1000:5000" "2:500:1000:3000"

# Формат параметра: "номер:ускорение:скорость:шаги"
poetry run python scripts/cli.py multi-move "1:500:1000:5000" "3:1000:2000:-3000"
```

## Multi-Move (движение нескольких моторов)

### Формат параметра

```
"номер_мотора:ускорение:макс_скорость:шаги"
```

| Поле | Диапазон | Описание |
|------|----------|----------|
| номер_мотора | 1-10 | Номер мотора |
| ускорение | > 0 | Ускорение (шаги/сек^2) |
| макс_скорость | > 0 | Максимальная скорость (шаги/сек) |
| шаги | любое | Количество шагов (отрицательные = реверс) |

### Примеры

```bash
# Два мотора с одинаковыми параметрами
poetry run python scripts/cli.py multi-move "1:500:1000:5000" "2:500:1000:5000"

# Разные параметры для разных моторов
poetry run python scripts/cli.py multi-move "1:500:1000:5000" "3:1000:2000:3000"

# Реверс (отрицательные шаги)
poetry run python scripts/cli.py multi-move "1:500:1000:-5000" "2:500:1000:5000"

# До 10 моторов одновременно
poetry run python scripts/cli.py multi-move "1:500:1000:1000" "2:500:1000:2000" \
    "3:500:1000:3000" "4:500:1000:4000" "5:500:1000:5000"
```

### Поведение

- По умолчанию используется **синхронный** режим (SYNC_MOVE)
- MCU последовательно отправляет команды на каждый драйвер через USART2
- Команда блокируется до завершения всех моторов
- Добавьте `--async` для асинхронного режима
